...
function M.parse(arg)
   local defaultDir = paths.concat(os.getenv('HOME'), 'fbcunn_imagenet')
    ...
    cmd:option('-inputSize',      221, 'spatial input size')
    cmd:option('-classes',     'none', 'Ordered class paths (lua file that returns a table)')
    cmd:option('-nonorm',       false, 'Turns off of normalization (zero-centered & unit-variance)')
    cmd:text()
